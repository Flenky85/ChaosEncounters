...
public class PartHealth : MechanicEntityPart, IInGameHandler<EntitySubscriber>, IInGameHandler, ISubscriber<IEntity>, ISubscriber, IEventTag<IInGameHandler, EntitySubscriber>, IUnitFeaturesHandler<EntitySubscriber>, IUnitFeaturesHandler, ISubscriber<IAbstractUnitEntity>, IEventTag<IUnitFeaturesHandler, EntitySubscriber>, IHashable
{
	...
	private int ApplyTemporaryHitPoints(int damage)
	{
		...
		foreach (TemporaryHitPointsData item in m_TemporaryHitPoints.ToTempList())
		{
			...
			{
				int damageValue = num;
				Handle(item, ref damageValue);
				damage = Math.Min(damageValue, damage);
			}
		}
		return damage;
		...
		void Handle(TemporaryHitPointsData tHP, ref int reference)
		{
			int value = tHP.Value;
			int num2 = reference - value;
			tHP.Value = ((num2 < 0) ? (-num2) : 0);
			reference = Math.Max(0, num2);
			if (tHP.Value <= 0)
			{
